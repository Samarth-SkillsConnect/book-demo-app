...
//     setLoadingBookings(true);
//     try {
//       // Make sure to send your admin token for protected endpoints
...
export default function AdminDashboard() {
  ...
  async function handleReschedule(id) {
    // Prompt for new date and time (can be replaced with a modal for better UX)
    ...
    setActionLoading(id);
    const token = localStorage.getItem("adminToken");
    await fetch(`http://localhost:5000/api/admin/bookings/${id}/reschedule`, {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
      body: JSON.stringify({
        date: newDate,
                          ...
                          {b.status === "pending" && (
                            <>
                              ...
                              <button
                                ...
                                disabled={actionLoading === b.id}
                              >
                                {actionLoading === b.id ? "Rescheduling..." : "Reschedule"}
                              </button>
                            </>
                          )}
                          {b.status === "accepted" && b.meet_link && (
