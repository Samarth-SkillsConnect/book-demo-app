...
exports.getRecurringSlotsConfig = async (req, res) => {
  const [rows] = await pool.query(
    // 'SELECT day_of_week as day, MIN(start_time) as start, MAX(end_time) as end, interval_minutes as `interval`, `status` FROM demo_slots GROUP BY day_of_week'
  `SELECT 
      day_of_week AS day, 
  ...
  // Map or format rows according to your frontend needs
  res.json(rows);
};
