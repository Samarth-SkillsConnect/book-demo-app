...
//  * Body: {
//  *   daysConfig: [
//  *     { day: "mon", status: "active"/"inactive"/"weekly-off", start, end, interval },
//  *     ...
//  *   ],
//  *   max_attendees: 10
//  * }
...
// exports.bulkCreateSlots = async (req, res) => {
//   try {
//     const { daysConfig, max_attendees = 10 } = req.body;
...
//       if (config.status === "weekly-off") {
//         // Optionally: store weekly-off status in a separate table or just skip
//         continue;
//       }
...
//             if (rows.length === 0) {
//               await pool.query(
//                 `INSERT INTO demo_slots (date, start_time, end_time, max_attendees, is_booked, day_of_week, day_status)
//                  VALUES (?, ?, ?, ?, 0, ?, ?)`,
//                 [slot.date, slot.start_time, slot.end_time, max_attendees, config.day, config.status]
//               );
//               createdCount++;
//             } else {
//               skippedCount++;
